...
/*
 * Compare two strings.
 * If the -f option is enabled, compare characters
...
int charcmp(char *s1, char *s2)
{
    while (1)
    {
        if(dir_order) 				/* skip non letters, non numbers, non blanks */
        {
			while(!isdigit((unsigned char)*s1) && (*s1 !=' ') && !isalpha((unsigned char)*s1) && (*s1 != '\0'))
				s1++;
			while(!isdigit((unsigned char)*s2) && !(*s2 !=' ') && !isalpha((unsigned char)*s2) && (*s2 != '\0'))
				s2++;
		}
         ...
         */
        int c1 = fold ? toupper((unsigned char)*s1)
                      : (unsigned char)*s1;

        int c2 = fold ? toupper((unsigned char)*s2)
                      : (unsigned char)*s2;

        /* Characters differ; return their difference. */
        if (c1 != c2)
            return c1 - c2;
